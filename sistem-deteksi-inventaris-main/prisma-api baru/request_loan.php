...
try {
    ...
    $checkAsset = $conn->prepare("SELECT asset_name, description, deskripsi, status, asset_code, QTY FROM assets WHERE id = ?");
    ...
    if (!empty($descriptionStr) && strpos($descriptionStr, '||META:') !== false) {
        ...
        foreach ($parts as $part) {
            if (strpos($part, 'META:') === 0) {
                ...
                if (isset($metaData['qty'])) {
                    $metaQty = (int)$metaData['qty'];
                }
            }
        }
    ...
    // Kolom QTY adalah sumber kebenaran utama (ini juga yang ditampilkan di frontend).
    // Encoding qty di description dipakai hanya sebagai fallback kalau QTY kosong/null.
    if (isset($asset['QTY']) && $asset['QTY'] !== null && $asset['QTY'] !== '') {
        $currentQty = (int)$asset['QTY'];
    } elseif ($metaQty !== null) {
        $currentQty = $metaQty;
    } else {
        $currentQty = 0;
    ...
    // --- PENGIRIMAN EMAIL DIISOLASI ---
    // Loan & update asset SUDAH sukses di titik ini. Kalau email gagal,
    // jangan sampai bikin seluruh request dianggap gagal oleh user.
    $emailSent = false;
    $emailError = null;
    ...
    try {
        ...
        $kodeAsetMail = isset($asset['asset_code']) ? $asset['asset_code'] : '-';
        ...
        // Jangan biarkan kegagalan email menggagalkan seluruh request.
        $emailError = $mailEx->getMessage();
        error_log("request_loan mail_helper error: " . $emailError);
    }

    $response = [
        "status" => "success",
        "message" => "Permintaan izin peminjaman alat berhasil diajukan!"
    ];
    if (!$emailSent && $emailError) {
        $response["email_warning"] = "Peminjaman berhasil, namun email konfirmasi gagal terkirim.";
    }
